...
enc28j60_config_t enc28j60_get_default_config() {
    enc28j60_config_t config = {
            .spi = spi0,
            .cs_pin = 6,
            .rcv_buffer_size = 4096,
            .mac_address = { 0x62, 0x5e, 0x22, 0x07, 0xde, 0x92 },
    };

    return config;
}

void enc28j60_read(enc28j60_config_t *config, uint8_t instruction, uint8_t *data, size_t len) {
...
uint16_t enc28j60_read_phy(enc28j60_config_t *config, uint8_t address) {
    enc28j60_switch_bank(config, 2);
...
void enc28j60_write_phy(enc28j60_config_t *config, uint8_t address, uint16_t data) {
    enc28j60_switch_bank(config, 2);
    enc28j60_write_cr8(config, ENC28J60_MIREGADR, address);
...
void enc28j60_init(enc28j60_config_t *config) {
    // LED setup
    enc28j60_write_phy(config, ENC28J60_PHLCON, 0x3476);

    // MAC setup
    enc28j60_switch_bank(config, 0);
    enc28j60_write_cr16(config, ENC28J60_ERXST, 0);  // Start receive buffer in 0 as per errata issue 5
    enc28j60_write_cr16(config, ENC28J60_ERXND, config->rcv_buffer_size - 1);
    enc28j60_write_cr16(config, ENC28J60_ERXRDPT, 0);

    enc28j60_switch_bank(config, 2);
    enc28j60_write_cr8(config, ENC28J60_MACON1, ENC28J60_MARXEN);
    enc28j60_write_cr8(config, ENC28J60_MACON3, ENC28J60_PADCFG_60 | ENC28J60_TXCRCEN | ENC28J60_FRMLNEN);
    enc28j60_write_cr8(config, ENC28J60_MACON4, ENC28J60_DEFER);
    enc28j60_write_cr16(config, ENC28J60_MAMXFL, 1500);
    enc28j60_write_cr8(config, ENC28J60_MABBIPG, 0x12);
    enc28j60_write_cr16(config, ENC28J60_MAIPG, 0x0C12);

    enc28j60_switch_bank(config, 3);
    enc28j60_write_cr16(config, ENC28J60_MAADR1, config->mac_address[0]);
    enc28j60_write_cr16(config, ENC28J60_MAADR2, config->mac_address[1]);
    enc28j60_write_cr16(config, ENC28J60_MAADR3, config->mac_address[2]);
    enc28j60_write_cr16(config, ENC28J60_MAADR4, config->mac_address[3]);
    enc28j60_write_cr16(config, ENC28J60_MAADR5, config->mac_address[4]);
    enc28j60_write_cr16(config, ENC28J60_MAADR6, config->mac_address[5]);

    // PHY setup
    enc28j60_write_phy(config, ENC28J60_PHCON2, ENC28J60_HDLDIS);  // Disable loopback as per errata issue 9

    // Enable interrupts
    enc28j60_bit_set(config, ENC28J60_EIE, ENC28J60_INTIE | ENC28J60_LINKIE);
}

void enc28j60_reset(enc28j60_config_t *config) {
    enc28j60_write(config, ENC28J60_SRC | ENC28J60_SRC_ARG, NULL, 0);
    sleep_ms(1);  // Errata issue 2
}

void enc28j60_receive_init(enc28j60_config_t *config) {
    // Enable interrupts
    enc28j60_bit_clear(config, ENC28J60_EIR, ENC28J60_PKTIF | ENC28J60_RXERIF);
    enc28j60_bit_set(config, ENC28J60_EIE, ENC28J60_PKTIE | ENC28J60_RXERIE);

    // Disable all filters
    enc28j60_switch_bank(config, 1);
    enc28j60_write_cr16(config, ENC28J60_ERXFCON, 0);

    // Enable reception
    enc28j60_bit_set(config, ENC28J60_ECON1, ENC28J60_RXEN);
}
...
size_t enc28j60_receive_packet(enc28j60_config_t *config, uint16_t *next_packet, uint8_t *buffer) {
    ...
    enc28j60_switch_bank(config, 0);
    enc28j60_write_cr16(config, ENC28J60_ERDPT, *next_packet);
    enc28j60_read(config, ENC28J60_RBM | 0x1a, (uint8_t *) &header, 6);

    *next_packet = header.next_packet;

    if (header.status & 0x80) {
        enc28j60_read(config, ENC28J60_RBM | 0x1a, buffer, header.byte_count - 4);

        uint32_t crc;
        enc28j60_read(config, ENC28J60_RBM | 0x1a, (uint8_t *) &crc, 4);
    }

    enc28j60_bit_set(config, ENC28J60_ECON2, ENC28J60_PKTDEC);

    // Free buffer & Errata issue 14
    if (*next_packet == enc28j60_read_cr8(config, ENC28J60_ERXST, false)) {
        enc28j60_write_cr16(config, ENC28J60_ERXRDPT, enc28j60_read_cr16(config, ENC28J60_ERXND));
    } else {
        enc28j60_write_cr16(config, ENC28J60_ERXRDPT, *next_packet - 1);
    }

    return header.byte_count - 4;
}

void enc28j60_transmit_packet(enc28j60_config_t *config, uint8_t *buffer, size_t len) {
    enc28j60_write_cr8(config, ENC28J60_ECON1, ENC28J60_RXEN | 0);
    enc28j60_write_cr16(config, ENC28J60_ETXST, config->rcv_buffer_size);
    enc28j60_write_cr16(config, ENC28J60_EWRPT, config->rcv_buffer_size);

    uint8_t control = 0;
    enc28j60_write(config, ENC28J60_WBM | 0x1a, &control, 1);
    enc28j60_write(config, ENC28J60_WBM | 0x1a, buffer, len);

    enc28j60_write_cr16(config, ENC28J60_ETXND, config->rcv_buffer_size + len);

    // Reset transmission logic, errata issue 12
    enc28j60_bit_set(config, ENC28J60_ECON1, ENC28J60_TXRST);
    enc28j60_bit_clear(config, ENC28J60_ECON1, ENC28J60_TXRST);

    enc28j60_bit_clear(config, ENC28J60_EIR, ENC28J60_TXIF);
    enc28j60_bit_set(config, ENC28J60_EIE, ENC28J60_TXIE | ENC28J60_INTIE);

    enc28j60_bit_set(config, ENC28J60_ECON1, ENC28J60_TXRTS);
    while (enc28j60_read_cr8(config, ENC28J60_ECON1, false) & ENC28J60_TXRTS);
    assert(!(enc28j60_read_cr8(config, ENC28J60_ESTAT, true) & ENC28J60_TXABRT));
}